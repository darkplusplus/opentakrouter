...
# windows
dotnet publish dpp.opentakrouter -c Release -r win10-x64 --self-contained=true -p:PublishSingleFile=true -p:PublishTrimmed=True -p:TrimMode=Link
pushd ./dpp.opentakrouter/bin/Release/net5.0/win10-x64/publish/
zip -r $DIST_DIR/opentakrouter-$VERSION-$HASH.zip .
popd

# linux
dotnet publish dpp.opentakrouter -c Release -r linux-x64 --self-contained=true -p:PublishSingleFile=true -p:PublishTrimmed=True -p:TrimMode=Link
